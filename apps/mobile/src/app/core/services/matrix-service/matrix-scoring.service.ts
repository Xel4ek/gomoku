...
//  consecutive 2's, 3's, 4's it has, how many of them are blocked etc...)
   ...
   */
  getScore(boardMatrix: BoardMatrix, forBlue: boolean, bluesTurn: boolean): number {

    // Calculate score for each of the 3 directions
    return this.evaluateHorizontal(boardMatrix, forBlue, bluesTurn) +
...
// The procedure is the exact same of the horizontal one.
  evaluateVertical(boardMatrix: BoardMatrix, forBlack: boolean, playersTurn: boolean): number {
...
// The procedure is the exact same of the horizontal calculation.
  evaluateDiagonal(boardMatrix: BoardMatrix, forBlack: boolean, playersTurn: boolean): number {
    ...
    for (let k = 0; k <= 2 * (boardMatrix.board.length - 1); k++) {
      let iStart = Math.max(0, k - boardMatrix.board.length + 1);
      let iEnd = Math.min(boardMatrix.board.length - 1, k);
    ...
    for (let k = 1 - boardMatrix.board.length; k < boardMatrix.board.length; k++) {
      let iStart = Math.max(0, k);
      let iEnd = Math.min(boardMatrix.board.length + k - 1, boardMatrix.board.length - 1);
      for (let i = iStart; i <= iEnd; ++i) {
        let j = i - k;
        const __ret = this.extracted(boardMatrix, i, j, forBlack, consecutive, blocks, score, playersTurn);
        consecutive = __ret.consecutive;
